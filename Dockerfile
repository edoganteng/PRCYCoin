...
RUN apt-get install -y libssl1.0-dev libzmq5 libevent-dev libboost-all-dev
RUN apt-get install -y libqt5gui5 libqt5core5a libqt5dbus5 qttools5-dev qttools5-dev-tools libqrencode-dev
RUN apt-get install -y software-properties-common g++-multilib binutils patch
RUN add-apt-repository ppa:pivx/pivx
RUN apt-get update
RUN apt-get install -y libdb4.8-dev libdb4.8++-dev
RUN ./autogen.sh
RUN ./configure --disable-jni --disable-tests --disable-gui-tests --disable-bench --enable-upnp-default
RUN make
RUN make install
EXPOSE 59682 59683 59684 59685
